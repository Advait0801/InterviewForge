...
Problem correctness itself is `scripts/verify_problems.py`; this covers the two
remaining Phase 7 checks, through the real HTTP API:
  ...
  2. Streak, activity and analytics queries still give correct numbers after real
...
def main():
    ...
    check(f"company + difficulty combine ({len(expected)} problems)", status == 200 and {p["slug"] for p in rows} == expected)
    ...
    check(f"company + topic combine ({len(expected)} problems)", status == 200 and {p["slug"] for p in rows} == expected)
    ...
    print("\n[2] stats, streak and analytics after real submissions")
